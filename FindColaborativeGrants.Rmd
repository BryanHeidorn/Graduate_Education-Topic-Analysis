...
MyData <- read.csv(filename, header=TRUE, encoding = "UTF-8", stringsAsFactors=FALSE) # tab delimited
```
...
```{r}
# Find the similar grant by abstract
MyData_Abs <- read.csv("duplicated.csv", header=TRUE, encoding = "UTF-8", stringsAsFactors=FALSE) # tab delimited
Mx_abs <- stringdistmatrix(tolower(MyData_Abs$Abstract), tolower(MyData_Abs$Abstract), method= c("lcs"))
# plot(hclust(as.dist(Mx)))
#serialization inspited by: https://stackoverflow.com/questions/27892100/distance-matrix-to-pairwise-distance-list-in-r
# Use the AwardNumber as the labels
dimnames(Mx_abs) <- dimnames(Mx_abs) <- list(MyData_Abs$AwardNumber, MyData_Abs$AwardNumber) 
#  Paste together the AwardNumber to the matrix
xy_abs <- t(combn(colnames(Mx_abs), 2))
MxSim_abs <- data.frame(xy_abs, dist=Mx_abs[xy_abs])
# Now sort by similarity and AwardNumber.
MxSim_abs <- MxSim_abs[ with(MxSim_abs, order(MxSim_abs$dist, MxSim_abs$X1)), ]
# Shorten the list to the most similar
ShortMxCol_abs <- MxSim_abs[ which(MxSim_abs$dist < 25), ]
# add the Project Titles 
# Add the title for the left AwardNumber
ShortMxCol_abs <- merge(x = ShortMxCol_abs, y = select(MyData_Abs, c(AwardNumber, Abstract)), by.y = c("AwardNumber"), by.x = "X1")
# Change the name of the "Title" Column to "Title 1"
colnames(ShortMxCol_abs)[which(names(ShortMxCol_abs) == "Abstract")] <- "Abstract 1"
# Add the title for the right AwardNumber
ShortMxCol_abs <- merge(x = ShortMxCol_abs, y = select(MyData_Abs, c(AwardNumber, Abstract)), by.y = c("AwardNumber"), by.x = "X2")
write.csv(ShortMxCol_abs, paste(topicDir,"/SimilarAbstract.csv", sep=""))
```
