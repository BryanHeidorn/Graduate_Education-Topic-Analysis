...
```{r}
# Read in the file
MyData <- read.csv("data/clean_edu.csv", header=TRUE, encoding = "UTF-8", stringsAsFactors=FALSE) # tab delimited
...
system.time(
  lemma_tagged <- treetag(lemma_unique$word_clean, treetagger="manual", 
                          ...
                          TT.options=list(
                          path="test/Applications/TreeTagger", preset="en"),
                           debug=TRUE
                          )
)
...
library(ggplot2)
p <- ggplot(subset(wf, freq>500), aes(reorder(term, -occurrences), occurrences))
...
wordcloud(names(freq),freq, min.freq=500,scale=c(4,.2), colors=brewer.pal(6,"Dark2"))
while (!is.null(dev.list()))  dev.off()
wordcloud(names(freq),freq, min.freq=500,scale=c(4,.2), colors=brewer.pal(6,"Dark2"))
...
```{r}
### this will map the topic numbers from the rest of the analysis to the multidimentional scaling topic numbers.
print("Original - MDS#")
...
p <- bigram_tf_idf %>%
  ...
  #ggplot(aes(tf_idf, bigram, fill = id)) +
  geom_col(show.legend = FALSE) +
...
#number_of_documents = 7 #number of top docs to view
...
# used for keep all documents whose gamma > 0.1
clean_gamma <- read.csv("data/clean_gamma.csv", header=TRUE, encoding = "UTF-8", stringsAsFactors= FALSE)
clean_gamma$document <- as.character(clean_gamma$document)
calculate_topic_gamma <- clean_gamma %>%
  group_by(topic) %>%
  arrange(topic, desc(gamma)) %>%
  mutate(row = row_number())
...
top_documents <- left_join(calculate_topic_gamma, top_bigrams_for_docs_wide,
...
nsf_amount <- data_frame(id = as.character(MyData$AwardNumber),
                            title = as.character(MyData$AwardedAmountToDate))
nsf_amount
...
Draw the funding amount across topics graph
```{r}
# create a new table which contains topic number and funding amoount
topic_funding <- TopicInvestmentBigram %>%
  select("topic", funding = WorthSum.x)

topic_funding <- topic_funding[!duplicated(topic_funding$topic),][1:25,]
write.csv(topic_funding, paste(topicDir,"/topic_funding.csv", sep=""))
topic_funding <- read.csv("data/topic_funding.csv", header=TRUE, encoding = "UTF-8", stringsAsFactors=FALSE)

ggplot(data=topic_funding, mapping = aes(x = reorder(topic, -funding), y = funding/1000)) +
  geom_bar(stat = "identity") +
  labs(x = "Topic Number",
       y = "Funding Amount($k)",
       title = "Funding Amount Across Each Topic Histogram")
```
